...
@section('title', 'Edit Route - Admin Panel')
    ...
    <div class="max-w-4xl mx-auto px-4">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-800">✏️ Edit Route</h1>
                    <p class="text-gray-600">Modify route details</p>
                </div>
            ...
            <form method="POST" action="{{ route('admin.routes.update', $route) }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="route_name" value="{{ old('route_name', $route->route_name) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g., Dhaka to Chittagong via Comilla">
                        @error('route_name')
                    ...
                    <div>
                        ...
                        <select name="start_station_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                            ...
                            @foreach($stations as $station)
                                <option value="{{ $station->id }}" {{ old('start_station_id', $route->start_station_id) == $station->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="end_station_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                            ...
                            @foreach($stations as $station)
                                <option value="{{ $station->id }}" {{ old('end_station_id', $route->end_station_id) == $station->id ? 'selected' : '' }}>
                                    {{ $station->station_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="total_distance_km" value="{{ old('total_distance_km', $route->total_distance_km) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g., 250.50">
                    ...
                    <div>
                        ...
                        <input type="number" name="estimated_duration_minutes" value="{{ old('estimated_duration_minutes', $route->estimated_duration_minutes) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g., 300">
                ...
                <div class="mt-8">
                    ...
                    <div id="intermediate-stations-container">
                        ...
                        @foreach($route->routeStations->sortBy('sequence_order') as $index => $routeStation)
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select name="intermediate_stations[{{ $index }}][station_id]" class="intermediate-station-select w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                                        ...
                                        @foreach($stations as $station)
                                            <option value="{{ $station->id }}" {{ $routeStation->station_id == $station->id ? 'selected' : '' }}>
                                                {{ $station->station_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <input type="number" name="intermediate_stations[{{ $index }}][sequence_order]" min="1" value="{{ old('intermediate_stations.' . $index . '.sequence_order', $routeStation->sequence_order) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g., 1, 2, 3">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Distance from Start (km)</label>
                                    <input type="number" step="0.01" name="intermediate_stations[{{ $index }}][distance_from_start_km]" value="{{ old('intermediate_stations.' . $index . '.distance_from_start_km', $routeStation->distance_from_start_km) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g., 50.25">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Arrival Time Offset (minutes)</label>
                                    <input type="number" name="intermediate_stations[{{ $index }}][arrival_time_offset_minutes]" min="0" value="{{ old('intermediate_stations.' . $index . '.arrival_time_offset_minutes', $routeStation->arrival_time_offset_minutes) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g., 60">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Departure Time Offset (minutes)</label>
                                    <input type="number" name="intermediate_stations[{{ $index }}][departure_time_offset_minutes]" min="0" value="{{ old('intermediate_stations.' . $index . '.departure_time_offset_minutes', $routeStation->departure_time_offset_minutes) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g., 65">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Halt Duration (minutes)</label>
                                    <input type="number" name="intermediate_stations[{{ $index }}][halt_duration_minutes]" min="0" value="{{ old('intermediate_stations.' . $index . '.halt_duration_minutes', $routeStation->halt_duration_minutes) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g., 5">
                                </div>
                                <div class="md:col-span-2">
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Platform Number</label>
                                    <input type="text" name="intermediate_stations[{{ $index }}][platform_number]" value="{{ old('intermediate_stations.' . $index . '.platform_number', $routeStation->platform_number) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g., 2">
                                </div>
                            </div>
                        </div>
                        @endforeach
