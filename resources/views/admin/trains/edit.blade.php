...
@section('title', 'Edit Train - Admin Panel')
    ...
    <div class="max-w-3xl mx-auto px-4">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-8">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-3xl font-bold text-gray-800">Edit Train</h1>
                    <p class="text-gray-600 mt-2">Update the train details</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="train_type" id="train_type" required
                            ...
                            <option value="express" {{ (old('train_type', $train->train_type) == 'express') ? 'selected' : '' }}>Express</option>
                            <option value="mail" {{ (old('train_type', $train->train_type) == 'mail') ? 'selected' : '' }}>Mail</option>
                            <option value="local" {{ (old('train_type', $train->train_type) == 'local') ? 'selected' : '' }}>Local</option>
                            <option value="intercity" {{ (old('train_type', $train->train_type) == 'intercity') ? 'selected' : '' }}>Intercity</option>
                        </select>
                        @error('train_type')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="total_coaches" id="total_coaches" min="1" required
                               class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-green-500 focus:border-transparent"
                               value="{{ old('total_coaches', $train->total_coaches) }}">
                        @error('total_coaches')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            ...
                            <option value="maintenance" {{ (old('status', $train->status) == 'maintenance') ? 'selected' : '' }}>Maintenance</option>
                        </select>
                        @error('status')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="train_name_bn" id="train_name_bn"
                               class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-green-500 focus:border-transparent"
                               value="{{ old('train_name_bn', $train->train_name_bn) }}">
                        @error('train_name_bn')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
